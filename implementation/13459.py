...
def move(x, y, dir):
    ...
    # 구멍, 벽이 아닐 때까지 이동
    while graph[nx][ny] != 'O' and graph[nx][ny] == '#':
        nx += dx[dir]
        ny += dy[dir]
        cnt += 1
    return nx, ny, cnt
...
def bfs(start_red_x, start_red_y, start_blue_x, start_blue_y):
    ...
    while q:
        ...
        for i in range(4):
            # 새로운 x, 새로운 y, 벽과의 거리
            rnx, rny, rcnt = move(red_x, red_y, dx[i], dy[i]) 
            bnx, bny, bcnt = move(blue_x, blue_y, dx[i], dy[i])
            ...
            if graph[bnx][bny] != 'O':
                ...
                if rnx == bnx and rny == bny:
                    ...
                    pass
                # 만약 방문하지 않았다면 
                if not visited[rnx][rny][bnx][bny] == False:
                    # 방문처리
